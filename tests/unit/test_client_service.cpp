...
protected:
    ...
    void TearDown() override {
        TestConfigHelper::cleanup(validConfigPath);
    }
...
TEST_F(ClientServiceTest, ConstructionWithInvalidConfigThrows) {
    ...
    EXPECT_THROW({
        ClientService service(invalidPath);
    }, std::runtime_error);
...
TEST_F(ClientServiceTest, HelloMessageFormatValid) {
    ...
    nlohmann::json helloPayload = {
        ...
        {"capabilities", nlohmann::json::array({"screenshot", "screen_lock", "power_control"})}
    };
    
    auto helloMsg = Message::Create(
        CommandType::HELLO,
        machineId,
        "master",
        helloPayload
