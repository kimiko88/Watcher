...
NetworkFilterManager::NetworkFilterManager(platform::INetworkFilter* filter, const std::string& config_path)
    : filter_(filter)
...
bool NetworkFilterManager::addBlockedDomain(const std::string& domain, RuleType type) {
            ...
            if (rule.type == RuleType::WILDCARD) {
                ...
                if (rule.domain.size() > 2 && rule.domain.substr(0, 2) == "*.") {
                    ...
                    if (normDomain.length() >= suffix.length()) {
                        if (normDomain.compare(normDomain.length() - suffix.length(), suffix.length(), suffix) == 0) {
                            return true;
                        }
                    }
                }
...
bool NetworkFilterManager::setFilterMode(FilterMode mode) {
    ...
    // Yes, usually setting mode implies enforcing it.
    // We cannot call applyRules() here easily if we hold the lock and applyRules takes the lock...
    // But applyRules needs to be called. 
    // Let's release lock or make applyRulesInternal.
    // For simplicity of this snippet, we will assume caller calls applyRules or we do it carefully.
...
bool NetworkFilterManager::loadRules() {
    ...
    if (!std::filesystem::exists(config_path_)) return false;
...
bool NetworkFilterManager::applyRules() {
    ...
    if (currentMode == FilterMode::MODE_DISABLED) {
        ...
        {
             std::lock_guard<std::mutex> lock(mutex_);
             for(const auto& r : rules_) if(r.action == Action::BLOCK) allBlocked.push_back(r.domain);
        }
...
std::string NetworkFilterManager::normalizeDomain(const std::string& domain) const {
