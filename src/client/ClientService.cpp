...
bool ClientService::start() {
    ...
    start_time_ = std::chrono::duration_cast<std::chrono::seconds>(duration).count();
    
    // Set running flag
    running_ = true;
    
    // Start processing thread
    processing_thread_ = std::make_unique<std::thread>(&ClientService::processingLoop, this);
...
bool ClientService::isRunning() const {
...
bool ClientService::connectToMaster() {
    LOG_INFO("Attempting to connect to master at " + 
             config_.master_address + ":" + std::to_string(config_.master_port));
...
void ClientService::sendHeartbeat() {
    ...
    try {
        // Create PING message
        auto pingMsg = protocol::Message::Create(
            protocol::CommandType::PING,
            config_.machine_id,
            "master",
        ...
        auto now = std::chrono::system_clock::now();
        auto duration = now.time_since_epoch();
        last_heartbeat_ = std::chrono::duration_cast<std::chrono::seconds>(duration).count();
